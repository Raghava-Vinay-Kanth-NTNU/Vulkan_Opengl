...
typedef struct {
    ...
    /*  the global transformation matrices */
    glm::mat4 projection;
    glm::mat4 view;
} CubeApp;
...
/* translate the debug message "severity" enum to human-readable string */
static const char *
 ...
 * This will initialize the app object, create a window and OpenGL context
 * (via GLFW), initialize the GL function pointers via GLAD and initialize
 * the cube.
...
bool initCubeApplication(CubeApp *app, const AppConfig& cfg)
{
    ...
    if (!gladLoadGLLoader((GLADloadproc)glfwGetProcAddress)) {
        warn("failed to initialize GLAD GL extension loader");
        return false;
    }
